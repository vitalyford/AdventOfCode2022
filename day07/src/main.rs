...
fn main() {
    ...
    read_to_string("./data.txt")
        ...
        .for_each(|line| {
            if cd_back.is_match(line) || line.trim().is_empty() {
                ...
                sum = -1;
                go_up.push_back(0);
                dirs.push_back(cd_forw.captures(line)
                                      .unwrap()
                                      .get(0)
                                      .map(|s| s.as_str())
                                      .unwrap());
    ...
    println!("Part 1: {}", out.iter().fold(0, |sum, e| if *e <= 100000 { sum + e } else { sum }));
    println!("Part 2: {}", out.iter().filter(|n| **n >= (30000000 - (70000000 - out[out.len() - 1]))).min().unwrap());
}
